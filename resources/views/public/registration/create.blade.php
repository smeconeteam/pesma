...
<div class="max-w-4xl mx-auto px-4 py-10">
    ...
    <form method="POST" action="{{ route('public.registration.store') }}" enctype="multipart/form-data" class="space-y-8">
        ...
        <section class="rounded-xl border bg-white p-6">
            ...
            <p class="text-sm text-gray-600 mt-1">Data akun untuk login resident (aktif setelah disetujui admin).</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-5">
                ...
                <div>
                    ...
                    <input name="name" type="text" value="{{ old('name') }}"
                           class="mt-1 w-full rounded-lg border-gray-300 focus:border-green-500 focus:ring-green-500" required>
                    <p class="text-xs text-gray-500 mt-1">Boleh sama dengan nama lengkap.</p>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <p class="text-xs text-gray-500 mt-1">Boleh dikosongkan, sistem akan set default seperti di admin (123456789).</p>
                </div>
            </div>
        </section>
        ...
        <section class="rounded-xl border bg-white p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-5">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-1">Jika WNI, otomatis Indonesia.</p>
                </div>
        ...
        <section class="rounded-xl border bg-white p-6">
            ...
            <p class="text-sm text-gray-600 mt-1">Tidak mengikat (sekadar informasi awal).</p>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-5">
                <div>
                    <label class="block text-sm font-medium">Cabang yang diinginkan</label>
                    <select name="preferred_dorm_id"
                            class="mt-1 w-full rounded-lg border-gray-300 focus:border-green-500 focus:ring-green-500">
                        <option value="">-- opsional --</option>
                        @foreach($dorms as $d)
                            <option value="{{ $d->id }}" @selected(old('preferred_dorm_id') == $d->id)>
                                {{ $d->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    <label class="block text-sm font-medium">Tipe kamar yang diinginkan</label>
                    <select name="preferred_room_type_id"
                            class="mt-1 w-full rounded-lg border-gray-300 focus:border-green-500 focus:ring-green-500">
                        <option value="">-- opsional --</option>
                        @foreach($roomTypes as $rt)
                            <option value="{{ $rt->id }}" @selected(old('preferred_room_type_id') == $rt->id)>
                                {{ $rt->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input name="planned_check_in_date" type="date"
                           value="{{ old('planned_check_in_date') }}"
                           class="mt-1 w-full rounded-lg border-gray-300 focus:border-green-500 focus:ring-green-500">
                </div>
            </div>
        </section>
