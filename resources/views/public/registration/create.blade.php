        ...
        <form method="POST" action="{{ localizedRoute('public.registration.store') }}" enctype="multipart/form-data" class="space-y-8">
            ...
            <section class="rounded-2xl border border-gray-100 bg-white p-6 shadow-lg sm:p-8 dark:border-gray-700 dark:bg-gray-800">
                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="relative">
                            <input id="password" name="password" type="password" value="123456789" class="w-full rounded-lg border-gray-300 pr-10 focus:border-green-500 focus:ring-green-500 dark:border-gray-600 dark:bg-gray-900 dark:text-white">

                            <button type="button" onclick="togglePasswordField()" class="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">
            ...
            <section class="rounded-2xl border border-gray-100 bg-white p-6 shadow-lg sm:p-8 dark:border-gray-700 dark:bg-gray-800">
                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <select id="preferred_dorm_id" name="preferred_dorm_id" class="{{ $fromRoom ? $disabledClass : '' }} w-full rounded-lg border-gray-300 focus:border-green-500 focus:ring-green-500 dark:border-gray-600 dark:bg-gray-900 dark:text-white" {{ $fromRoom ? 'disabled' : '' }}>
                            <option value="">{{ __('registration.select_branch') }}</option>
                            @foreach ($dorms as $d)
                                <option value="{{ $d->id }}" @selected(old('preferred_dorm_id', $prefill['preferred_dorm_id']) == $d->id)>
                                    {{ $d->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        @endif
                        <p id="dorm-info" class="mt-2 hidden text-xs font-medium"></p>
                    </div>

                    {{-- Tipe Kamar --}}
                    <div>
                        <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('registration.room_type') }}</label>
                        <select id="preferred_room_type_id" name="preferred_room_type_id" class="{{ $fromRoom ? $disabledClass : '' }} w-full rounded-lg border-gray-300 focus:border-green-500 focus:ring-green-500 dark:border-gray-600 dark:bg-gray-900 dark:text-white" {{ $fromRoom ? 'disabled' : '' }}>
                            <option value="">{{ __('registration.select_room_type') }}</option>
                            @foreach ($roomTypes as $rt)
                                <option value="{{ $rt->id }}" @selected(old('preferred_room_type_id', $prefill['preferred_room_type_id']) == $rt->id)>
                                    {{ $rt->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        @endif
                        <p id="room-type-info" class="mt-2 hidden text-xs font-medium"></p>
                    </div>

                    {{-- Nomor Kamar --}}
                    <div class="md:col-span-2">
                        <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">
                            {{ __('registration.room_number') }} <span class="font-normal text-gray-400">{{ __('registration.optional') }}</span>
                        </label>

                        @if ($fromRoom)
                            {{-- Dari halaman detail kamar: tampil read-only --}}
                            <div class="{{ $disabledClass }} flex w-full items-center gap-3 rounded-lg border border-gray-300 bg-gray-100 px-3 py-2.5 dark:border-gray-600 dark:bg-gray-700">
                        ...
                        @else
                            ...
                            <select id="preferred_room_id" name="preferred_room_id" class="w-full rounded-lg border-gray-300 focus:border-green-500 focus:ring-green-500 dark:border-gray-600 dark:bg-gray-900 dark:text-white">
            ...
            @if ($policy)
                <div class="flex items-start space-x-3 rounded-xl border border-green-200 bg-green-50 p-5 dark:border-green-900/40 dark:bg-green-900/20">
                    <input type="checkbox" name="agreed_to_policy" id="agreed_to_policy" value="1" class="mt-1 h-5 w-5 flex-shrink-0 cursor-pointer rounded border-gray-300 text-green-600 focus:ring-green-500 dark:border-gray-600 dark:bg-gray-800 dark:ring-offset-gray-900" {{ old('agreed_to_policy') ? 'checked' : '' }} required>
                    <label for="agreed_to_policy" class="cursor-pointer text-sm text-gray-700 dark:text-gray-300">
                        {{ __('registration.policy_agree') }}
                        <a href="{{ localizedRoute('public.policy') }}" target="_blank" rel="opener" class="font-semibold text-green-700 underline decoration-2 underline-offset-2 hover:text-green-800 dark:text-green-400 dark:hover:text-green-300">
                ...
                @error('agreed_to_policy')
                    <p class="mt-2 pl-8 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            @else
            ...
            <div class="flex items-center justify-end gap-3 pt-4">
                ...
                <button id="submitBtn" type="submit" class="rounded-lg bg-green-600 px-6 py-2.5 text-center font-medium text-white shadow-md transition-all hover:bg-green-700 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-green-500 dark:hover:bg-green-600 dark:focus:ring-offset-gray-900" @if (!$policy) disabled title="{{ __('registration.policy_unavailable') }}" @endif>
