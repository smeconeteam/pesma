        ...
        <div>
            ...
            <div class="flex flex-wrap justify-center gap-8">
                @foreach ($dorms as $dorm)
                    @foreach ($dorm->adminScopes as $scope)
                        @php
                            $admin = $scope->user;
                            ...
                            <div class="grow space-y-4">
                                <div class="flex items-center justify-between gap-4 rounded-xl border border-gray-100 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                                    <div class="flex items-center gap-4">
                                        <div class="h-12 w-12 shrink-0 overflow-hidden rounded-full border-2 border-gray-100 bg-gray-100 dark:border-gray-700 dark:bg-gray-700">
                                            @if ($photo)
                                                <img src="{{ Storage::url($photo) }}" alt="{{ $admin->name }}" class="h-full w-full object-cover">
                                            @else
                                    ...
                                    @if ($phone !== '-')
                                        <a href="https://wa.me/{{ $waNumber }}" target="_blank" class="shrink-0 rounded-full bg-green-100 p-2 text-green-600 transition-colors hover:bg-green-200 dark:bg-green-900/30 dark:text-green-400 dark:hover:bg-green-900/50" title="{{ __('contact.contact_via_whatsapp') }}">
