<x-public-layout>
    ...
    <div class="mx-auto max-w-7xl py-12">
        <!-- Breadcrumb -->
        <div class="mb-6 flex gap-2 text-sm text-gray-600 px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-10 px-4 sm:px-6 lg:px-8">
            <h1 class="mb-2 text-4xl font-bold">{{ __('public.rooms_available') }}</h1>
            <p class="text-lg text-gray-600">{{ __('public.find_dream_room', ['count' => $rooms->total()]) }}</p>
        </div>

        <!-- Search and Filter Section -->
        <div class="rounded-2xl bg-white p-6 relative">
            <form method="GET" action="{{ route('rooms.available') }}" class="flex flex-wrap items-center gap-4">
                ...
                <div class="relative min-w-[280px] flex-1">
                    ...
                    <input type="text" name="search" class="pl-12! w-full rounded-xl border border-gray-200 py-4 pr-4" placeholder="{{ __('public.search_placeholder') }}" value="{{ request('search') }}" oninput="debounceSearch()">
                </div>

                <!-- Filter Dropdown (FilamentPHP Style) -->
                <div x-data="{ open: false }" @click.away="open = false" class="">
                    <!-- Filter Button -->
                    <button type="button" @click="open = !open" class="inline-flex items-center gap-2 rounded-xl border-2 border-gray-100 px-3 py-2.5 transition-all duration-200 hover:border-gray-300 hover:bg-gray-50">
                        ...
                        <span class="text-sm font-medium text-gray-700">{{ __('public.filters') }}</span>
                    ...
                    <!-- Dropdown Panel -->
                    <div x-show="open" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" class="absolute right-0 z-50 mt-2 w-80 rounded-xl border border-gray-200 bg-white shadow-xl" style="display: none;">
                        <div class="p-4">
                            <div class="mb-4 flex items-center justify-between border-b border-gray-100 pb-3">
                                <h3 class="font-semibold text-gray-900">{{ __('public.filters') }}</h3>
                            ...
                            <div class="space-y-4">
                                <!-- Branch Filter -->
                                <div>
                                    <label class="mb-2 block text-sm font-medium text-gray-700">{{ __('public.branch') }}</label>
                                    <select name="dorm_id" class="w-full cursor-pointer rounded-lg border border-gray-200 px-3 py-2.5 text-sm transition-colors hover:border-gray-300 focus:border-green-500 focus:outline-none focus:ring-2 focus:ring-green-500/20">
                                ...
                                <!-- Room Type Filter -->
                                <div>
                                    <label class="mb-2 block text-sm font-medium text-gray-700">{{ __('public.room_type') }}</label>
                                    <select name="room_type_id" class="w-full cursor-pointer rounded-lg border border-gray-200 px-3 py-2.5 text-sm transition-colors hover:border-gray-300 focus:border-green-500 focus:outline-none focus:ring-2 focus:ring-green-500/20">
                                ...
                                <!-- Category Filter -->
                                <div>
                                    <label class="mb-2 block text-sm font-medium text-gray-700">{{ __('public.type') }}</label>
                                    <select name="resident_category_id" class="w-full cursor-pointer rounded-lg border border-gray-200 px-3 py-2.5 text-sm transition-colors hover:border-gray-300 focus:border-green-500 focus:outline-none focus:ring-2 focus:ring-green-500/20">
        ...
        <!-- Rooms Container -->
        <div id="rooms-container" class="mx-auto mt-4 flex flex-col items-center justify-between gap-6 rounded-t-2xl bg-white p-4 sm:p-6 sm lg:p-8">
