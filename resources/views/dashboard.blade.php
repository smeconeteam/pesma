<x-app-layout>
    ...
    <div class="py-4 sm:py-8 bg-gray-50 min-h-screen">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-4 sm:space-y-6">
                ...
                <div class="p-5 sm:p-7">
                    <div class="flex items-start gap-3 sm:gap-4">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            @if (!$isInactive && $roomPicName)
                                <div class="mt-2 flex items-center gap-2 text-xs sm:text-sm text-gray-700">
                                    <svg class="w-4 h-4 text-blue-600 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
                                    </svg>
                                    <span class="font-medium">Ketua Kamar:</span>
                                    <span class="font-bold text-gray-900">{{ $roomPicName }}</span>
                                </div>
                            @endif
            ...
            {{-- KONTAK PENTING --}}
            @if ($contacts->count() > 0 || ($roomPicName && $roomPicPhone))
            <div class="bg-white overflow-hidden shadow-lg sm:rounded-2xl rounded-xl border border-gray-100">
                <div class="p-5 sm:p-7">
                    ...
                    <div class="space-y-3">
                        {{-- PIC Kamar --}}
                        @if ($roomPicName && $roomPicPhone)
                            <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $roomPicPhone) }}?text=Halo Kak {{ $roomPicName }}, saya penghuni kamar {{ $roomCode }}" 
                            target="_blank"
                            class="group flex items-center gap-3 sm:gap-4 p-4 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl border-2 border-blue-200 hover:border-blue-400 hover:shadow-lg hover:scale-[1.02] transition-all duration-200">
                                <div class="p-2.5 sm:p-3 bg-blue-600 group-hover:bg-blue-700 rounded-lg transition-colors shadow-md shrink-0">
                                    <svg class="w-5 h-5 sm:w-6 sm:h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
                                    </svg>
                                </div>
                                <div class="flex-1">
                                    <div class="font-bold text-gray-900 text-sm sm:text-base group-hover:text-blue-700 transition-colors">
                                        {{ $roomPicName }} (Ketua Kamar)
                                    </div>
                                    <div class="text-xs sm:text-sm text-gray-600 mt-0.5 font-medium">
                                        {{ $roomPicPhone }}
                                    </div>
                                </div>
                                <svg class="w-5 h-5 sm:w-6 sm:h-6 text-blue-600 opacity-0 group-hover:opacity-100 transition-opacity shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                                </svg>
                            </a>
                        @endif
                        ...
                        target="_blank"
                        class="group flex items-center gap-3 sm:gap-4 p-4 bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl border-2 border-green-200 hover:border-green-400 hover:shadow-lg hover:scale-[1.02] transition-all duration-200">
                            <div class="p-2.5 sm:p-3 bg-green-600 group-hover:bg-green-700 rounded-lg transition-colors shadow-md shrink-0">
                            ...
                            <div class="flex-1">
                                <div class="font-bold text-gray-900 text-sm sm:text-base group-hover:text-green-700 transition-colors">
                            ...
                            <svg class="w-5 h-5 sm:w-6 sm:h-6 text-green-600 opacity-0 group-hover:opacity-100 transition-opacity shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                            </svg>
                        </a>
                        @endforeach
